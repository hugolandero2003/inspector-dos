...
export default function AdminPage() {
  ...
  // Auth por cookie — obtiene el token desde la sesión del servidor
  const cookieAuth = {
  ...
  useEffect(() => {
    // Verificar sesión por cookie en lugar de localStorage
  ...
  const handleLogout = () => {
    fetch("/api/auth/logout").finally(() => router.push("/login"));
  };
  ...
  const handleSaveEdit = async () => {
    if (!editingRecord || !editForm || !sessionToken) {
      return;
    }
            ...
            <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-2xl flex flex-col gap-5">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowShareModal(false)}
                  className="rounded-lg p-1.5 text-[var(--muted)] hover:bg-[var(--surface-strong)] transition">
                  <XMarkIcon className="h-5 w-5" />
                </button>
              </div>
              <div className="flex flex-col gap-3">
                <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Compartir vía</p>
                <a href={`https://wa.me/?text=${encodeURIComponent(`Hola 👋 Por favor registra tu inspección preoperacional diaria usando este enlace:\n${typeof window !== "undefined" ? window.location.origin : ""}/sistema`)}`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-sm font-semibold text-green-600 transition hover:bg-green-500/20">
                ...
                <a href={`mailto:?subject=${encodeURIComponent("Formulario de inspección preoperacional")}&body=${encodeURIComponent(`Por favor registra tu inspección diaria:\n\n${typeof window !== "undefined" ? window.location.origin : ""}/sistema`)}`}
                ...
                <a href="/sistema" target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm font-semibold text-blue-500 transition hover:bg-blue-500/20">
                  <svg className="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
